...
-- depends_on: {{ ref('stg_shopify_tst__orders__fulfillments') }}
...
/* select which columns will be available for table 'orders__fulfillments__line_items' */
    ...
    "_dlt_id",
    "variant_title",
FROM {{ source('raw_data', 'orders__fulfillments__line_items') }}

/* we only load table items if the parent table has the parent item */
WHERE "_dlt_parent_id" IN ( SELECT "_dlt_id" FROM {{ ref('stg_shopify_tst__orders__fulfillments') }} )
