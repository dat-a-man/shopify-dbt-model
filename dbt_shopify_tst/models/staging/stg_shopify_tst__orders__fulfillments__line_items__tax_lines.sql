...
-- depends_on: {{ ref('stg_shopify_tst__orders__fulfillments__line_items') }}
...
/* select which columns will be available for table 'orders__fulfillments__line_items__tax_lines' */
    ...
    "_dlt_list_idx",
    "_dlt_id",
FROM {{ source('raw_data', 'orders__fulfillments__line_items__tax_lines') }}

/* we only load table items if the parent table has the parent item */
WHERE "_dlt_parent_id" IN ( SELECT "_dlt_id" FROM {{ ref('stg_shopify_tst__orders__fulfillments__line_items') }} )
